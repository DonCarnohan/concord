...
class CommonFields(models.Model):
    created_by_user = models.ForeignKey(settings.AUTH_USER_MODEL, related_name="+")
    modified_by_user = models.ForeignKey(settings.AUTH_USER_MODEL, related_name="+", blank=True, null=True)
    deleted_by_user = models.ForeignKey(settings.AUTH_USER_MODEL, related_name="+", blank=True, null=True)
...
class UserProfile(CommonFields):
    ...
    options_json = models.TextField(blank=False, default='{}')
...
class Group(CommonFields):
    ...
    url_name = models.CharField(blank=False, default='', max_length=10, unique=True)
    ...
    home_page_markdown = models.TextField(blank=True, default='')
    ...
    def has_member(self, user):
        return self.groupmember_set.filter(user=user).exists()

class GroupMember(CommonFields):
    user = models.ForeignKey(settings.AUTH_USER_MODEL)
    group = models.ForeignKey(Group)
    display_name = models.TextField(blank=False, default='')
...
class EventCategory(CommonFields):
    description = models.TextField(blank=False, default='')
    group = models.ForeignKey(Group)
    permissions_json = models.TextField(blank=False, default='{}')
...
class Event(CommonFields):
    ...
    extra_information = models.TextField(blank=False, default='{}')
    permissions_json = models.TextField(blank=False, default='{}')
...
class InviteCode(CommonFields):
    group = models.ForeignKey(Group)
    code = models.TextField(blank=False, default='')
    valid_until_timestamp = models.DateTimeField()

    class Meta:
        db_table = 'invite_code'
        verbose_name = ugettext_lazy('Invite Code')
